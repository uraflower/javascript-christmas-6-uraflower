...
- [ ] 입력 받은 날짜를 가지고 방문 날짜를 생성한다. (ex. `2023년 12월 x일`)
  - 예외
    ...
    - [ ] 입력 값이 `1~31` 사이 정수가 아닌 경우 (ex. `1일`, `-1`, `1.0`, `5.0000000000000001`, `hi`)

### 날짜 정보 전달

- [ ] 생성한 날짜의 날을 전달한다. (getter nono)
- [ ] 생성한 날짜의 요일을 전달한다. (getter nono)
- [ ] 생성한 날짜가 이벤트 달력에 표시된 별 날짜인지 여부를 전달한다. (getter nono)
...
- 입력 받은 메뉴와 개수를 검증한다.
  - 예외
    ...
    - [ ] 주문한 메뉴가 메뉴판에 없는 경우
    - [ ] 메뉴의 각 개수가 양의 정수가 아닌 경우
    - [ ] 입력 형식에 맞지 않는 경우
    - [ ] 메뉴를 중복 입력한 경우 (ex. 시저샐러드-1,시저샐러드-1)
  ...
  - 예외 (메뉴 개수 제한 관련 안내)
    ...
    - [ ] 주문한 메뉴의 총 개수가 `20`을 초과한 경우

## ✔ 할인

### 크리스마스 디데이 할인 금액 계산

- [ ] 방문 날짜가 `1~25` 사이인 경우, `1000원 + ((일수 - 1) * 100원)` 만큼 할인한다.

### 평일 할인 금액 계산

- [ ] 방문 날짜가 `1~31` 사이이면서 방문 요일이 `일요일~목요일` 사이인 경우, 디저트 메뉴 1개당 `2023원` 만큼 할인한다.

### 주말 할인 금액 계산

- [ ] 방문 날짜가 `1~31` 사이이면서 방문 요일이 `금요일~토요일` 사이인 경우, 메인 메뉴 1개당 `2023원` 만큼 할인한다.

### 특별 할인 금액 계산

- [ ] 방문 날짜가 `1~31` 사이이면서 이벤트 달력에 별(🌟)이 있는 날짜인 경우, `1000원` 만큼 할인한다.
...
- [ ] 증정 기준에 부합하는 경우, 증정 메뉴의 이름과 개수를 확인한다.
  ...
  ```
  할인 전 총주문 금액이 `12만 원` 이상인 경우, 샴페인 1개를 증전한다.
  ```
...
- [ ] 주문 받은 메뉴를 출력한다. 출력 순서는 자유롭게 해도 된다.
    - 출력 형식: `{메뉴 이름} {개수}개`
