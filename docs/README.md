...
- [x] 조건을 만족할 경우, `1000원 + ((일수 - 1) * 100원)`만큼 할인한다.
...
- [x] 증정 기준에 부합하는 경우, 증정 메뉴의 정보를 저장한다.
  
  ```
  총주문 금액이 1만 원 이상, 방문 날짜가 1일~31일이면서...
  - 할인 전 총주문 금액이 12만 원 이상: 샴페인 1개
  ```

- [x] 증정 기준에 부합하지 않는 경우, 증정 메뉴가 없음을 확인한다.
...
- [x] 총 혜택 금액을 불러와 기준에 맞는 배지 이름을 전달한다.
...
- [x] 배지 증정 이벤트에 해당되지 않는 경우, 증정 배지가 없음을 확인한다.
